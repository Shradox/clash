...
function moveBridge(u, c, tgt, dt) {
  ...
  // p1 goes: spawn → WP1(bx, 2.0) → WP2(bx, -2.0) → target
  // p2 goes: spawn → WP1(bx,-2.0) → WP2(bx,  2.0) → target
  // Waypoints are well outside the river zone so no "almost there" stalling
  const bx = u.bridge;
  const wp1z = u.side==='p1' ?  2.0 : -2.0;  // bridge entry (own side)
  const wp2z = u.side==='p1' ? -2.0 :  2.0;  // bridge exit  (enemy side)

  // Determine current waypoint based on progress
  let gx, gz;

  const atWP1 = Math.abs(u.x - bx) < 0.5 && (u.side==='p1' ? u.z <= wp1z + 0.3 : u.z >= wp1z - 0.3);
  const atWP2 = Math.abs(u.x - bx) < 0.5 && (u.side==='p1' ? u.z <= wp2z + 0.3 : u.z >= wp2z - 0.3);

  if (!atWP1) {
    // Phase 1: go to bridge entry on own side
    gx = bx;
    gz = wp1z;
  } else if (!atWP2) {
    // Phase 2: cross river along bridge x
    gx = bx;
    gz = wp2z;
  } else {
    // Phase 3: head straight to target
    if (!tgt) return;
    gx = tgt.x;
    gz = tgt.z;
  }
  ...
  if (len > 0.05) { u.x += dx/len*spd; u.z += dz/len*spd; }
}
