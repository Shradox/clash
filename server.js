...
const OVERTIME_DURATION = 60; // доп. время (сек)
const ELIXIR_REGEN_OT = 0.7;  // эликсир быстрее в овертайме
...
// ── AI SOLO — создаём фиктивный ws-like объект для бота ──
...
// AI tick — вызывается из gameTick для ai игрока
function tickAI(g) {
  const aiSide = g.players[1] && g.players[1].isAI ? 'p2' : null;
  if (!aiSide) return;
  if (!g._aiTimer) g._aiTimer = 2 + Math.random() * 2;
  // Используем dt из последнего вызова gameTick — храним в g
  g._aiTimer -= g._lastDt || 0.033;
  if (g._aiTimer > 0) return;

  // Выбираем случайную карту которую AI может купить
  const affordable = CARDS.filter(c => c.cost <= g.elixir[aiSide] && c.type !== 'spell');
  if (!affordable.length) { g._aiTimer = 1; return; }
  const card = affordable[Math.floor(Math.random() * affordable.length)];

  // AI тоже должен иметь колоду
  if (!g._aiHand) {
    const d = shuffle(CARDS.length);
    g._aiHand = d.slice(0,4).map(i=>CARDS[i].id);
  }

  const x = (Math.random() - 0.5) * 10;
  const z = -(Math.random() * 8 + 1.5);
  placeCard(g, aiSide, card.id, x, z);
  g._aiTimer = 2.5 + Math.random() * 3;
}
...
function startGame(p1, p2) {
  ...
  const g = {
    ...
    events: [],
    interval: null,
    overtime: false,        // флаг овертайма
    overtimeLeft: OVERTIME_DURATION,
    totalDmgToEnemy: {p1:0, p2:0}, // суммарный урон по чужим башням
    stars: {p1:0, p2:0},   // 1=одна башня, 2=две, 3=главная
  };
...
function mkUnit(cardId, side, x, z) {
  ...
  return { uid:nextUid++, cardId, side, x, z,
    ...
    bridge: x < 0 ? BRIDGE_L : BRIDGE_R,  // left half → left bridge, right half → right
    dead:false };
}
...
function placeCard(g, side, cardId, x, z) {
  ...
  let hi = g.hand[side].indexOf(ci);
  // AI may not have a formal hand — allow bypass
  if (hi<0) {
    if (g.players.find(p=>p.side===side&&p.isAI)) hi=0; // AI: use slot 0
    else return;
  }
...
function castSpell(g, c, side, tx, tz) {
  ...
  if (c.ability==='spell_aoe') {
    ...
    // Lightning hits the highest-HP enemy within 5.5 radius of click
    const LTG_RANGE = 5.5;
    const targets=[
      ...g.units.filter(u=>u.side===enemySide&&!u.dead&&dist2d(u.x,u.z,tx,tz)<=LTG_RANGE),
      ...g.towers.filter(t=>t.side===enemySide&&!t.destroyed&&dist2d(t.x,t.z,tx,tz)<=LTG_RANGE)
    ];
    if(targets.length){
      const top=targets.reduce((a,b)=>a.hp>b.hp?a:b);
      top.hp-=500;
      g.events.push({type:'lightning',x:top.x,z:top.z,cx:tx,cz:tz});
      checkDeath(g,top);
    } else {
      // Nothing in range — visual only
  ...
  if(!g.overtime){
    ...
    if(g.timeLeft <= 0){
      ...
      if(g.crowns.p1 !== g.crowns.p2){
        // Разные кол-во корон — побеждает тот у кого больше
        endGame(g, g.crowns.p1>g.crowns.p2?'p1':'p2'); return;
      }
      // Ничья по коронам — запускаем овертайм
      g.overtime=true;
      g.overtimeLeft=OVERTIME_DURATION;
      broadcast(g,{type:'overtime_start'});
      console.log(`[G${g.id}] OVERTIME started`);
    }
  } else {
    // Овертайм: ускоренный элексир уже выше, считаем время
    g.overtimeLeft -= dt;
    if(g.overtimeLeft <= 0){
      g.overtimeLeft=0;
      // Определяем победителя по нанесённому урону башням (у кого меньше HP осталось у врага)
      // Считаем суммарное оставшееся HP вражеских башен для каждого
      const p1EnemyHpLeft = g.towers.filter(t=>t.side==='p2'&&!t.destroyed).reduce((a,t)=>a+Math.max(0,t.hp),0);
      const p2EnemyHpLeft = g.towers.filter(t=>t.side==='p1'&&!t.destroyed).reduce((a,t)=>a+Math.max(0,t.hp),0);
      // Победитель — тот кто нанёс больше урона (меньше HP осталось у врага)
      let w=null;
      if(p1EnemyHpLeft < p2EnemyHpLeft) w='p1';
      else if(p2EnemyHpLeft < p1EnemyHpLeft) w='p2';
      endGame(g,w); return;
    }
  }
  // Ускоренный элексир в овертайме
  const eRegen = g.overtime ? ELIXIR_REGEN_OT : ELIXIR_REGEN;
  g.elixir.p1 = Math.min(10, g.elixir.p1 + eRegen * dt);
  g.elixir.p2 = Math.min(10, g.elixir.p2 + eRegen * dt);

  // Бот
  g._lastDt = dt;
  if (g.players[1] && g.players[1].isAI) tickAI(g);
...
function applyDmg(g,tgt,dmg,attackerSide){
  if(tgt.destroyed!==undefined){
    // башня
    const actualDmg=Math.min(dmg,Math.max(0,tgt.hp));
    tgt.hp-=dmg;
    // Track damage to enemy towers for tiebreaker
    if(attackerSide&&tgt.side!==attackerSide){
      g.totalDmgToEnemy[attackerSide]=(g.totalDmgToEnemy[attackerSide]||0)+actualDmg;
    }
    checkTowerDeath(g,tgt);
  } else {
    // юнит
...
function moveBridge(g,u,dt,c){
  ...
  let mx,mz;
  if(pastRiver){
    // Past river — go straight to target, keep bridge assignment
    mx=gx; mz=gz;
  } else if(inRiver){
    // In river: strictly follow bridge X corridor, walk through
    u.x=u.x*0.7+bx*0.3;  // snap toward bridge x
    mx=bx; mz=u.side==='p1'?RIVER_MIN-0.1:RIVER_MAX+0.1;
  } else {
    // Approach bridge entry: slide toward bridge X, then enter river
    const eZ=u.side==='p1'?RIVER_MAX+0.2:RIVER_MIN-0.2;
    const alignThresh=0.5;  // wider threshold so units don't get stuck
    if(Math.abs(px-bx)>alignThresh){
      // Slide sideways toward bridge, keep current Z
      mx=bx; mz=pz;
    } else {
      // Aligned with bridge — walk into river zone
      mx=bx; mz=eZ;
    }
  }
